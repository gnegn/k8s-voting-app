.PHONY: help \
	...
	argocd-install argocd-password argocd-port-forward \
	monitoring-grafana monitoring-prometheus grafana-password \
	eks-create eks-destroy eks-kubeconfig \
...
# ── Full local setup ──────────────────────────────────────────
up: ## Build → cluster → load → deploy → status
	$(MAKE) docker-build
	$(MAKE) kind-create
	$(MAKE) docker-load
	$(MAKE) argocd-install
...
# ── Argo CD ───────────────────────────────────────────────────
argocd-install: ## Install Argo CD and apply app-of-apps
